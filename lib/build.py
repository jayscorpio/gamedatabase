...
def do_translate(dryrunonly):
    ...
    for folder_to_build in TRANSLATION_FOLDERS:
        translate_folder(BASE_PATH+'/src/'+ folder_to_build, dryrunonly)
    printline()
    print('Done creating translated files at '+BUILD_FOLDER_NAME)
    printline()

def do_build():
    mkdir(COLLECTIONS_FOLDER_NAME)
    print('building translated files into single jsonArray for mongoimport at '+BUILD_FOLDER_NAME+"/"+COLLECTIONS_FOLDER_NAME)

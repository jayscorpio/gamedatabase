...
def do_build():
    ...
    mPrint(
        "building translated files into single jsonArray for mongoimport at "
        + BUILD_FOLDER_NAME
        + "/"
        + COLLECTIONS_FOLDER_NAME
    )

    build_folder_folders = [
        f.name
        for f in os.scandir(BUILD_FOLDER)
        if f.is_dir() and not f.name == COLLECTIONS_FOLDER_NAME
    ]

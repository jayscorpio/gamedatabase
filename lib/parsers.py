...
def materials_parser(filename, data, get_translation_specific):
    ...
    if "shops" in data and len(data["shops"]) > 0:
        try:
            for shop_i in range(len(data["shops"])):
                if "chapter" in data["shops"][shop_i]:
                    data["shops"][shop_i]["chapter"] = get_translation_specific(
                        data["shops"][shop_i]["chapter"]
                    )
                if "continent" in data["shops"][shop_i]:
                    data["shops"][shop_i]["continent"] = get_translation_specific(
                        data["shops"][shop_i]["continent"]
                    )
        except:
            pass
    return data
...
def hero_parser(filename, data, get_translation_specific):
    ...
    data["assets"] = {}
    data["assets.icon"] = lib.utils.mount_assets("face/", data["id"] + "_s", "png")
    data["assets.image"] = lib.utils.mount_assets("face/", data["id"] + "_su", "png")
    data["assets.thumbnail"] = lib.utils.mount_assets("face/", data["id"] + "_l", "png")
    ...
    if data["camping"]:
        ...
        try:
            ...
            for camp_values in camp_val_keys:
                camp_val_new[get_translation_specific(camp_values)] = data[
                    "camping.values." + camp_values
                ]
    ...
    for zodiac_in in range(len(data["zodiac_tree"])):
        ...
        for zodiac_cost_index in range(len(data["zodiac_tree"][zodiac_in]["costs"])):
            data["zodiac_tree"][zodiac_in]["costs"][zodiac_cost_index][
                "_id"
            ] = "zodiac_{0}_cost_{1}".format(zodiac_in, zodiac_cost_index)
    ...
    for sk_in in range(len(data["skills"])):
        ...
        try:
            for sk_enh_index in range(len(data["skills"][sk_in]["enhancements"])):
                data["skills"][sk_in]["enhancements"][sk_enh_index][
                    "_id"
                ] = "skill_{0}_enh_{1}".format(sk_in, sk_enh_index)
                data["skills"][sk_in]["enhancements"][sk_enh_index][
                    "string"
                ] = get_translation_specific(
                    data["skills"][sk_in]["enhancements"][sk_enh_index]["string"]
                )

                for sk_enh_cost_index in range(
                    len(data["skills"][sk_in]["enhancements"][sk_enh_index]["costs"])
                ):
                    data["skills"][sk_in]["enhancements"][sk_enh_index]["costs"][
                        sk_enh_cost_index
                    ]["_id"] = "skill_{0}_enh_{1}_cost_{2}".format(
    ...
    if data["specialty_change"]:
        if "quests" in data["specialty_change"] and data["specialty_change.quests"] and len(data["specialty_change.quests"]) > 0:
            for sc_in in range(len(data["specialty_change.quests"])):
                data["specialty_change.quests"][sc_in][
                    "category"
                ] = get_translation_specific(
        ...
        if "tree" in data["specialty_change"] and data["specialty_change.tree"] and len(data["specialty_change.tree"]) > 0:
            for sc_tree_in in range(len(data["specialty_change.tree"])):
                for sc_tree_row_in in range(
                    len(data["specialty_change.tree"][sc_tree_in])
                ):
                    for sc_tree_row_enhancements_in in range(
                        len(
                            data["specialty_change.tree"][sc_tree_in][sc_tree_row_in][
                                "enhancements"
                            ]
                        )
                    ):
                        currentEnhancement = data["specialty_change.tree"][sc_tree_in][
                            sc_tree_row_in
                        ]["enhancements"][sc_tree_row_enhancements_in]
    ...
    if data["relationships"] and len(data["relationships"]) > 0:
        for relation_in in range(len(data["relationships"])):
            ...
            if data["relationships"][relation_in]["description"]:
                data["relationships"][relation_in][
                    "description"
                ] = get_translation_specific(
                    data["relationships"][relation_in]["description"]
                )
            if (
                data["relationships"][relation_in]["relations"]
                and len(data["relationships"][relation_in]["relations"]) > 0
            ):
                for relationship_relation_in in range(
                    len(data["relationships"][relation_in]["relations"])
                ):
                    try:
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["description"] = get_translation_specific(
                            data["relationships"][relation_in]["relations"][
                                relationship_relation_in
                            ]["description"]
                        )
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["relation_id"] = data["relationships"][relation_in][
                            "relations"
                        ][
                            relationship_relation_in
                        ][
                        ...
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["relation"] = get_translation_specific(
                            data["relationships"][relation_in]["relations"][
                                relationship_relation_in
                            ]["relation"]
                        )
                    ...
                    try:
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["upgrade"]["description"] = get_translation_specific(
                            data["relationships"][relation_in]["relations"][
                                relationship_relation_in
                            ]["upgrade"]["description"]
                        )
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["upgrade"]["relation_id"] = data["relationships"][
                            relation_in
                        ][
                            "relations"
                        ][
                        ...
                        data["relationships"][relation_in]["relations"][
                            relationship_relation_in
                        ]["upgrade"]["relation"] = get_translation_specific(
                            data["relationships"][relation_in]["relations"][
                                relationship_relation_in
                            ]["upgrade"]["relation"]
                        )
